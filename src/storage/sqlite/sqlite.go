...
func (s *SQLiteStorage) UpdateQueueAttributes(ctx context.Context, queueName string, attributes map[string]string) error {
	...
	_, err := s.db.ExecContext(ctx,
		"UPDATE queues SET attributes = ? WHERE name = ?",
		string(attributesJSON), queueName)
	if err != nil {
...
func (s *SQLiteStorage) ReceiveMessages(ctx context.Context, queueName string, maxMessages int, waitTimeSeconds int) ([]*storage.Message, error) {
	now := time.Now()
	...
	for rows.Next() {
		...
		message.ReceiptHandle = uuid.New().String()
		message.VisibilityTimeout = now.Add(30 * time.Second)
